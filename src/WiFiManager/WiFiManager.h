...
const char HTTP_PORTAL_OPTIONS[] PROGMEM  = "<br/><form action=\"/wifi\" method=\"get\"><button>Настройки</button></form><br/><form action=\"/r\" method=\"post\"><button>Очистить настройки и данные сопряжения</button></form>";
...
const char HTTP_FORM_RAIN_PARAM_DISCLAIMER[] PROGMEM      = "<div><h2>Настройки параметров датчика дождя.</h2> </br> Допустимые значения 0-1023. Установите значение 9999, чтобы отключить датчик дождя.</div>";
...
const char HTTP_SAVED[] PROGMEM           = "<div style='width: 50%'>Параметры сохранены.<br/>Сейчас девайс попытается подключиться к заданной Wi-Fi сети.<br/>Если подключение не удасться, девайс создаст свою Wi-Fi сеть вновь и будет ожидать конфигурирования.</div>";
